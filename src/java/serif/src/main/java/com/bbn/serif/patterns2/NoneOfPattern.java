...
/**
 * A pattern which matches if none of its sub-patterns match on a single extraction unit.
 *
 ...
 * Example:
 * <pre>
 *   (none-of (members (mention (acetype PER))
 *   (mention (headword company troop unit))))
 *   </pre>
 *
 *   selects mentions that are neither of type {@code PER} nor whose headword is
 *   <i>company/troop/unit</i>
 *
 *
 ...
 *
 * Note that while you can construct an {@code NoneOfPattern}, you cannot currently execute one.
 * For that, see issue #331
 */
@Beta
@TextGroupImmutable
public abstract class NoneOfPattern implements Pattern {
